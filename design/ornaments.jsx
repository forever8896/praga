// Praga — Rudolfine ornaments
// FleurDeLis, WaxSeal, Cartouche, AlchemicalSigil, Marginalia

// ============== FleurDeLis ==============
// Three-petal, slightly elongated, line-art only — no fills.
function FleurDeLis({ size = 24, stroke = 'var(--gilded)', strokeWidth = 0.8, style = {} }) {
  return (
    <svg width={size} height={size} viewBox="0 0 40 40" style={{ display: 'block', ...style }} fill="none" stroke={stroke} strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round">
      {/* central spear */}
      <path d="M20 4 C 20 12, 20 22, 20 28" />
      {/* central bud at top */}
      <path d="M20 4 C 18.5 6, 18.5 8, 20 9 C 21.5 8, 21.5 6, 20 4 Z" />
      {/* left petal — curls outward then under */}
      <path d="M20 14 C 14 14, 9 18, 8 24 C 7 28, 10 30, 13 28 C 15.5 26.5, 17 22, 20 20" />
      {/* right petal */}
      <path d="M20 14 C 26 14, 31 18, 32 24 C 33 28, 30 30, 27 28 C 24.5 26.5, 23 22, 20 20" />
      {/* horizontal sash */}
      <path d="M11 22 C 15 21, 25 21, 29 22" />
      {/* base flourish */}
      <path d="M14 28 C 16 30, 24 30, 26 28" />
      <path d="M16 30 L 24 30" />
      <path d="M20 28 L 20 33" />
    </svg>
  );
}

// ============== WaxSeal ==============
// Off-axis, with subtle cracks. Used for verification + receipts + escrow states.
function WaxSeal({
  size = 90,
  state = 'rubedo', // nigredo | albedo | citrinitas | rubedo | broken
  rotate = -6,
  emboss = 'fleur', // fleur | crescent | sun | none
  label,
  style = {},
}) {
  const palettes = {
    nigredo:    { wax: '#1A1814', edge: '#0A0908', seal: '#3A332A', text: '#7B6F58' },
    albedo:     { wax: '#1A1814', edge: '#0A0908', seal: '#E6DCC4', text: '#E6DCC4' },
    citrinitas: { wax: '#221F18', edge: '#100E09', seal: '#C8A24A', text: '#C8A24A' },
    rubedo:     { wax: '#B23A2F', edge: '#7A2218', seal: '#C8A24A', text: '#F4ECD8' },
    broken:     { wax: '#7A2218', edge: '#3A0F0A', seal: '#3A0F0A', text: '#F4ECD8' },
  };
  const p = palettes[state] || palettes.rubedo;
  const r = 50;
  // subtle irregular edge — fixed seed for stability
  const edgePts = [];
  const N = 36;
  const seed = state.length;
  for (let i = 0; i < N; i++) {
    const a = (i / N) * Math.PI * 2;
    const noise = (Math.sin(i * 1.7 + seed) * 0.5 + Math.cos(i * 2.3 + seed * 2) * 0.5) * 1.6;
    const rad = r + noise;
    edgePts.push(`${50 + Math.cos(a) * rad},${50 + Math.sin(a) * rad}`);
  }
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" style={{ display: 'block', transform: `rotate(${rotate}deg)`, ...style }}>
      <defs>
        <radialGradient id={`waxgrad-${state}-${rotate}`} cx="40%" cy="35%" r="65%">
          <stop offset="0%" stopColor={p.wax} stopOpacity="1" />
          <stop offset="60%" stopColor={p.wax} stopOpacity="1" />
          <stop offset="100%" stopColor={p.edge} stopOpacity="1" />
        </radialGradient>
        <filter id={`waxshadow-${state}-${rotate}`}>
          <feGaussianBlur stdDeviation="0.8" />
        </filter>
      </defs>
      {state !== 'broken' ? (
        <>
          <polygon points={edgePts.join(' ')} fill={`url(#waxgrad-${state}-${rotate})`} stroke={p.edge} strokeWidth="0.4" />
          {/* highlight smear */}
          <ellipse cx="38" cy="32" rx="14" ry="6" fill={p.seal} opacity="0.07" />
          {/* cracks */}
          <path d="M 22 50 L 30 52 L 28 55" stroke={p.edge} strokeWidth="0.3" fill="none" opacity="0.5" />
          <path d="M 78 60 L 72 62" stroke={p.edge} strokeWidth="0.3" fill="none" opacity="0.4" />
          {/* inner ring */}
          <circle cx="50" cy="50" r="38" fill="none" stroke={p.seal} strokeWidth="0.4" opacity="0.5" />
          <circle cx="50" cy="50" r="35" fill="none" stroke={p.seal} strokeWidth="0.3" opacity="0.3" />
          {/* emboss */}
          <g transform="translate(50,50)">
            {emboss === 'fleur' && (
              <g transform="translate(-14,-14)" stroke={p.seal} strokeWidth="0.7" fill="none" strokeLinecap="round" opacity="0.95">
                <path d="M14 2 C 14 8, 14 18, 14 22" />
                <path d="M14 2 C 13 4, 13 6, 14 7 C 15 6, 15 4, 14 2 Z" />
                <path d="M14 11 C 9 11, 5 14, 4 19 C 3.2 22, 5.5 23.5, 8 22 C 10 21, 11 17.5, 14 16" />
                <path d="M14 11 C 19 11, 23 14, 24 19 C 24.8 22, 22.5 23.5, 20 22 C 18 21, 17 17.5, 14 16" />
                <path d="M7 17 C 10 16, 18 16, 21 17" />
                <path d="M9 22 C 11 24, 17 24, 19 22" />
                <path d="M11 24 L 17 24" />
              </g>
            )}
            {emboss === 'crescent' && (
              <path d="M -10 -8 A 12 12 0 1 0 -10 8 A 9 9 0 1 1 -10 -8 Z" fill={p.seal} opacity="0.95" />
            )}
            {emboss === 'sun' && (
              <g stroke={p.seal} strokeWidth="0.8" fill="none" opacity="0.95">
                <circle cx="0" cy="0" r="9" />
                <circle cx="0" cy="0" r="2" fill={p.seal} />
                {Array.from({ length: 12 }).map((_, i) => {
                  const a = (i / 12) * Math.PI * 2;
                  return <line key={i} x1={Math.cos(a) * 11} y1={Math.sin(a) * 11} x2={Math.cos(a) * 14} y2={Math.sin(a) * 14} />;
                })}
              </g>
            )}
          </g>
          {/* tiny perimeter text */}
          {label && (
            <g style={{ fontFamily: 'var(--display)', fontSize: 5, letterSpacing: '0.18em', fill: p.text, textTransform: 'uppercase' }} opacity="0.85">
              <defs>
                <path id={`waxpath-${state}-${rotate}`} d="M 50 50 m -42 0 a 42 42 0 1 1 84 0 a 42 42 0 1 1 -84 0" />
              </defs>
              <text>
                <textPath href={`#waxpath-${state}-${rotate}`} startOffset="25%" textAnchor="middle">{label}</textPath>
              </text>
            </g>
          )}
        </>
      ) : (
        <>
          {/* broken seal — two halves */}
          <g transform="translate(-3,0)">
            <polygon points={edgePts.slice(0, N / 2 + 2).join(' ') + ' 50,50'} fill={`url(#waxgrad-${state}-${rotate})`} stroke={p.edge} strokeWidth="0.4" />
          </g>
          <g transform="translate(3,1)">
            <polygon points={edgePts.slice(N / 2).join(' ') + ' 50,50'} fill={`url(#waxgrad-${state}-${rotate})`} stroke={p.edge} strokeWidth="0.4" />
          </g>
          <path d="M 50 8 L 47 50 L 53 50 L 50 92" stroke={p.edge} strokeWidth="0.6" fill="none" opacity="0.7" />
        </>
      )}
    </svg>
  );
}

// ============== Cartouche ==============
// Asymmetric scrollwork frame, gilded hairline. Supports rectangular cards.
function Cartouche({ children, style = {}, padding = 24, accent = 'var(--gilded)', tone = 'parchment', tight = false }) {
  const bg = tone === 'parchment' ? 'transparent' : tone === 'bone' ? 'var(--bone)' : tone === 'ink' ? 'var(--ink)' : tone;
  return (
    <div style={{ position: 'relative', background: bg, padding, ...style }}>
      <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: 'none' }} preserveAspectRatio="none" viewBox="0 0 100 100">
        {/* outer hairline */}
        <rect x="1.5" y="1.5" width="97" height="97" fill="none" stroke={accent} strokeWidth="0.18" vectorEffect="non-scaling-stroke" />
        {/* inner hairline */}
        <rect x="3" y="3" width="94" height="94" fill="none" stroke={accent} strokeWidth="0.12" vectorEffect="non-scaling-stroke" opacity="0.5" />
      </svg>
      {/* corner flourishes — drawn separately so they keep aspect */}
      {!tight && (
        <>
          <CartoucheCorner pos="tl" accent={accent} />
          <CartoucheCorner pos="tr" accent={accent} />
          <CartoucheCorner pos="bl" accent={accent} />
          <CartoucheCorner pos="br" accent={accent} />
        </>
      )}
      <div style={{ position: 'relative' }}>{children}</div>
    </div>
  );
}

function CartoucheCorner({ pos, accent }) {
  const transforms = {
    tl: 'translate(0,0)',
    tr: 'translate(100%,0) scale(-1,1)',
    bl: 'translate(0,100%) scale(1,-1)',
    br: 'translate(100%,100%) scale(-1,-1)',
  };
  const offsets = {
    tl: { top: 0, left: 0 },
    tr: { top: 0, right: 0 },
    bl: { bottom: 0, left: 0 },
    br: { bottom: 0, right: 0 },
  };
  return (
    <svg width="36" height="36" viewBox="0 0 40 40" style={{ position: 'absolute', ...offsets[pos], pointerEvents: 'none' }}>
      <g transform={transforms[pos].includes('scale') ? transforms[pos].replace('100%', '40').replace('100%', '40') : ''} stroke={accent} strokeWidth="0.6" fill="none" strokeLinecap="round">
        {/* scrollwork — asymmetric */}
        <path d="M 4 14 C 8 12, 12 10, 14 4" />
        <path d="M 6 18 C 10 16, 16 12, 18 6" opacity="0.6" />
        <path d="M 4 14 C 5 16, 7 16, 8 14 C 8 12, 6 12, 6 14" />
        <path d="M 14 4 C 16 5, 16 7, 14 8 C 12 8, 12 6, 14 6" />
        <circle cx="6" cy="14" r="0.6" fill={accent} />
        <circle cx="14" cy="6" r="0.6" fill={accent} />
      </g>
    </svg>
  );
}

// ============== Alchemical Sigils (category icons) ==============
// Line engravings inside thin gilded circular hairline frames.
const SIGIL_PATHS = {
  // ♂ Mars-and-anvil — handyman / repair
  forge: (
    <g stroke="currentColor" strokeWidth="0.9" fill="none" strokeLinecap="round" strokeLinejoin="round">
      <circle cx="20" cy="22" r="6" />
      <line x1="24.5" y1="17.5" x2="30" y2="12" />
      <path d="M 27 12 L 30 12 L 30 15" />
      <rect x="14" y="30" width="12" height="2.5" />
      <path d="M 12 32.5 L 10 35 M 28 32.5 L 30 35" />
    </g>
  ),
  // ☿ Mercury — language exchange
  mercury: (
    <g stroke="currentColor" strokeWidth="0.9" fill="none" strokeLinecap="round" strokeLinejoin="round">
      <path d="M 14 8 C 14 12, 17 14, 20 14 C 23 14, 26 12, 26 8" />
      <circle cx="20" cy="20" r="5" />
      <line x1="20" y1="25" x2="20" y2="32" />
      <line x1="16" y1="29" x2="24" y2="29" />
    </g>
  ),
  // 🜍 Sulphur over flame — cooking
  sulphur: (
    <g stroke="currentColor" strokeWidth="0.9" fill="none" strokeLinecap="round" strokeLinejoin="round">
      <path d="M 20 8 L 14 18 L 26 18 Z" />
      <line x1="20" y1="18" x2="20" y2="26" />
      <path d="M 16 26 L 24 26 M 17 28 L 23 28" />
      {/* flame */}
      <path d="M 16 32 C 16 30, 18 30, 18 32 C 18 30, 20 30, 20 32 C 20 30, 22 30, 22 32 C 22 30, 24 30, 24 32" />
    </g>
  ),
  // ⚕ winged caduceus — rides / transit
  caduceus: (
    <g stroke="currentColor" strokeWidth="0.9" fill="none" strokeLinecap="round" strokeLinejoin="round">
      <line x1="20" y1="8" x2="20" y2="32" />
      <path d="M 20 12 C 16 14, 16 18, 20 20 C 24 22, 24 26, 20 28" />
      <path d="M 20 12 C 24 14, 24 18, 20 20 C 16 22, 16 26, 20 28" />
      <path d="M 12 11 C 14 9, 17 9, 19 11" />
      <path d="M 28 11 C 26 9, 23 9, 21 11" />
      <circle cx="20" cy="9" r="0.8" fill="currentColor" />
    </g>
  ),
  // ♄ Saturn with owl — tutoring
  saturn: (
    <g stroke="currentColor" strokeWidth="0.9" fill="none" strokeLinecap="round" strokeLinejoin="round">
      <path d="M 16 10 L 24 10" />
      <line x1="20" y1="10" x2="20" y2="22" />
      <path d="M 20 22 C 17 22, 15 24, 15 27 C 15 30, 17 32, 19 31 C 18 28, 20 26, 22 28" />
      {/* tiny owl perched */}
      <ellipse cx="28" cy="28" rx="2.5" ry="3" />
      <circle cx="27" cy="27" r="0.5" fill="currentColor" />
      <circle cx="29" cy="27" r="0.5" fill="currentColor" />
      <path d="M 26.5 25.5 L 27 27 M 29.5 25.5 L 29 27" />
    </g>
  ),
  // ♀ Venus rose — gifts / freebies
  venus: (
    <g stroke="currentColor" strokeWidth="0.9" fill="none" strokeLinecap="round" strokeLinejoin="round">
      <circle cx="20" cy="16" r="6" />
      <line x1="20" y1="22" x2="20" y2="32" />
      <line x1="16" y1="28" x2="24" y2="28" />
      {/* rose inside */}
      <path d="M 20 14 C 18 14, 18 17, 20 17 C 22 17, 22 14, 20 14 Z" />
      <path d="M 17 16 C 18 14, 22 14, 23 16" opacity="0.6" />
    </g>
  ),
  // ⚗ alembic — errands
  alembic: (
    <g stroke="currentColor" strokeWidth="0.9" fill="none" strokeLinecap="round" strokeLinejoin="round">
      <path d="M 17 10 L 23 10 L 23 14 L 27 22 C 28 24, 27 26, 24 26 L 16 26 C 13 26, 12 24, 13 22 L 17 14 Z" />
      <line x1="15" y1="10" x2="25" y2="10" />
      <path d="M 23 14 L 32 14 L 32 22 L 30 22" />
      <path d="M 18 22 C 20 21, 22 21, 24 22" />
    </g>
  ),
};

function AlchemicalSigil({ kind = 'forge', size = 40, frame = true, color = 'var(--ink)', frameColor = 'var(--gilded)', style = {} }) {
  return (
    <span style={{ display: 'inline-flex', width: size, height: size, ...style }}>
      <svg width={size} height={size} viewBox="0 0 40 40" style={{ color, display: 'block' }}>
        {frame && <circle cx="20" cy="20" r="18.5" fill="none" stroke={frameColor} strokeWidth="0.5" opacity="0.85" />}
        {SIGIL_PATHS[kind] || SIGIL_PATHS.forge}
      </svg>
    </span>
  );
}

// ============== Marginalia ==============
// Decorative marginal sketches — astrological glyphs, tiny diagrams.
// Decorative-only; never interactive.
function Marginalia({ kind, size = 80, style = {} }) {
  const stroke = 'var(--ink)';
  const sw = 0.5;
  const items = {
    // squared circle (alchemical philosopher's stone)
    squaredcircle: (
      <g fill="none" stroke={stroke} strokeWidth={sw} opacity="0.55">
        <rect x="10" y="10" width="80" height="80" />
        <circle cx="50" cy="50" r="38" />
        <polygon points="50,16 84,68 16,68" />
        <circle cx="50" cy="50" r="14" />
        <text x="50" y="52" textAnchor="middle" style={{ fontFamily: 'var(--display)', fontSize: 7, fill: stroke }}>VIIIIVIII</text>
      </g>
    ),
    // tiny constellation (Prague tram route abstracted)
    constellation: (
      <g fill={stroke} stroke={stroke} strokeWidth={sw} opacity="0.6">
        <line x1="14" y1="22" x2="34" y2="40" />
        <line x1="34" y1="40" x2="58" y2="32" />
        <line x1="58" y1="32" x2="72" y2="58" />
        <line x1="72" y1="58" x2="50" y2="78" />
        <line x1="50" y1="78" x2="22" y2="68" />
        {[[14,22],[34,40],[58,32],[72,58],[50,78],[22,68]].map(([x,y],i)=> <circle key={i} cx={x} cy={y} r="1.6" fill={stroke} />)}
        <text x="50" y="14" textAnchor="middle" style={{ fontFamily: 'var(--display)', fontSize: 6, fill: stroke }}>ŽIŽKOV · KARLÍN · VINOHRADY</text>
      </g>
    ),
    // astrological — sun + ringed planet
    astrolog: (
      <g fill="none" stroke={stroke} strokeWidth={sw} opacity="0.6">
        <circle cx="28" cy="30" r="8" />
        <circle cx="28" cy="30" r="1.5" fill={stroke} />
        {Array.from({ length: 8 }).map((_, i) => {
          const a = (i / 8) * Math.PI * 2;
          return <line key={i} x1={28 + Math.cos(a) * 10} y1={30 + Math.sin(a) * 10} x2={28 + Math.cos(a) * 13} y2={30 + Math.sin(a) * 13} />;
        })}
        <ellipse cx="68" cy="62" rx="10" ry="3" transform="rotate(-22 68 62)" />
        <circle cx="68" cy="62" r="6" />
      </g>
    ),
    // small Prague map
    pragueMap: (
      <g fill="none" stroke={stroke} strokeWidth={sw} opacity="0.6">
        <path d="M 12 60 C 28 50, 40 70, 60 56 C 76 46, 88 60, 88 70" />
        <path d="M 12 60 C 28 70, 40 50, 60 64" opacity="0.5" />
        {/* castle silhouette */}
        <path d="M 52 32 L 56 28 L 56 24 L 60 24 L 60 28 L 64 28 L 64 24 L 68 24 L 68 28 L 72 32 L 72 40 L 52 40 Z" />
        <line x1="58" y1="22" x2="58" y2="24" />
        <line x1="66" y1="22" x2="66" y2="24" />
        <circle cx="36" cy="58" r="1.5" fill="var(--vermilion)" stroke="none" />
        <text x="40" y="60" style={{ fontFamily: 'var(--mono)', fontSize: 5, fill: stroke }}>here</text>
      </g>
    ),
    // alembic diagram
    alembicDiagram: (
      <g fill="none" stroke={stroke} strokeWidth={sw} opacity="0.6">
        <path d="M 30 24 L 60 24 L 60 38 L 72 64 C 76 72, 70 78, 60 78 L 30 78 C 20 78, 14 72, 18 64 L 30 38 Z" />
        <line x1="26" y1="24" x2="64" y2="24" />
        <path d="M 60 38 L 86 38 L 86 60 L 80 60" />
        <path d="M 38 64 C 42 60, 50 60, 54 64" />
        <text x="50" y="92" textAnchor="middle" style={{ fontFamily: 'var(--display)', fontSize: 6, fill: stroke, letterSpacing: '0.1em' }}>ALEMBICUM</text>
      </g>
    ),
    // hand-drawn fleur
    fleurSketch: (
      <g fill="none" stroke={stroke} strokeWidth={sw * 1.2} opacity="0.55" strokeLinecap="round">
        <path d="M 50 14 C 50 32, 50 56, 50 72" />
        <path d="M 50 14 C 46 18, 46 22, 50 24 C 54 22, 54 18, 50 14 Z" />
        <path d="M 50 36 C 36 36, 24 44, 22 60 C 20 70, 28 74, 36 70 C 42 66, 46 56, 50 50" />
        <path d="M 50 36 C 64 36, 76 44, 78 60 C 80 70, 72 74, 64 70 C 58 66, 54 56, 50 50" />
        <path d="M 28 56 C 38 54, 62 54, 72 56" />
        <path d="M 32 72 C 40 78, 60 78, 68 72" />
        <path d="M 36 76 L 64 76" />
      </g>
    ),
    // celestial chart
    chart: (
      <g fill="none" stroke={stroke} strokeWidth={sw} opacity="0.55">
        <circle cx="50" cy="50" r="38" />
        <circle cx="50" cy="50" r="28" />
        {Array.from({ length: 12 }).map((_, i) => {
          const a = (i / 12) * Math.PI * 2;
          return <line key={i} x1={50 + Math.cos(a) * 28} y1={50 + Math.sin(a) * 28} x2={50 + Math.cos(a) * 38} y2={50 + Math.sin(a) * 38} />;
        })}
        <line x1="12" y1="50" x2="88" y2="50" />
        <line x1="50" y1="12" x2="50" y2="88" />
        <text x="50" y="8" textAnchor="middle" style={{ fontFamily: 'var(--display)', fontSize: 5, fill: stroke }}>ANNO MMXXVI</text>
      </g>
    ),
  };
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" style={{ display: 'block', ...style }}>
      {items[kind]}
    </svg>
  );
}

// ============== Quill icon (edit affordance) ==============
function Quill({ size = 16, color = 'var(--gilded)' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none" stroke={color} strokeWidth="1" strokeLinecap="round" strokeLinejoin="round">
      <path d="M 3 17 L 14 6 C 16 4, 17 4, 17 6 C 17 8, 15 9, 13 11 L 4 18 Z" />
      <line x1="3" y1="17" x2="6" y2="14" />
      <path d="M 11 8 L 13 10" />
    </svg>
  );
}

// ============== Stamped category chip ==============
function CategoryChip({ kind, label, active = false }) {
  return (
    <span style={{
      display: 'inline-flex', alignItems: 'center', gap: 6,
      padding: '6px 12px',
      background: active ? 'var(--ink)' : 'transparent',
      color: active ? 'var(--parchment)' : 'var(--ink)',
      border: '0.5px solid var(--gilded)',
      borderRadius: 999,
      fontFamily: 'var(--display)',
      fontSize: 11,
      letterSpacing: '0.15em',
      textTransform: 'uppercase',
      fontWeight: 500,
    }}>
      <AlchemicalSigil kind={kind} size={16} frame={false} color={active ? 'var(--parchment)' : 'var(--ink)'} />
      {label}
    </span>
  );
}

Object.assign(window, {
  FleurDeLis, WaxSeal, Cartouche, AlchemicalSigil, Marginalia, Quill, CategoryChip,
});
