# PragueConnect — ETHPrague Hackathon Plan
...
### Product
Design a mobile-first web app called **PragueConnect** (working name) — a peer-to-peer marketplace where humans in Prague offer micro-skills, favors, and gifts to each other for small crypto payments. Think TaskRabbit × Craigslist gigs × ENS-native identity, but the unit of trust is a *person*, not a platform.
...
1. **Landing / Onboarding** — "Claim your name in Prague." User connects wallet, gets/links an ENS subname (e.g., `kilian.pragueconnect.eth`), which becomes both their profile URL and their personal site.
...
3. **Profile / personal site** — the page rendered at `username.pragueconnect.eth.limo`. Bio, skills offered, reputation, completed tasks, gifts given. This is *their* site — they own it. Show a "you are viewing your own page" state and a "public visitor" state.
...
6. **Reputation / completed task receipt** — what a finished gig looks like. Mutual rating, on-chain attestation badge, "verified by PragueConnect" stamp. Reputation must feel *earned and personal*, not gamified-points-y.
7. **Edit-your-site dashboard** — the secret authenticated view where the ENS-name owner manages their personal page, subname records, and which data is public vs. private. Make the "subnames as data shelves" concept visually legible (e.g., `bio.kilian.pragueconnect.eth`, `skills.kilian.pragueconnect.eth`, `inbox.kilian.pragueconnect.eth`).
...
- **Subnames as a feature, not a footnote.** The personal site at `name.pragueconnect.eth.limo` and the subname-shelves model should be obviously different from "yet another username."
...
**Track target: Network Economy.** Track copy ("next-generation privacy, identity, and on-chain economic solutions putting user control first … real economic and privacy challenges in a decentralized way") maps line-for-line onto PragueConnect. Their own example — *zk identity wallet with selective verifiable claim sharing* — is the same shape as PragueConnect's ZK reputation proofs. Don't split focus into Future Society (their example is a zk-IoT environmental oracle, not us).

### 1. ENS Bounty 2 — *Most Creative Use of ENS*  ($1,250 / $750)

**Confirmed winning examples (their words):**
- **Stealth addresses (EIP-5564), auto-rotating addresses per resolution, dual-key ECDH** ← PragueConnect's payments layer hits this directly
...
**Native fit:** very strong. PragueConnect can plausibly hit *four* of these bullets in one feature surface:
...
3. Contenthash personal site at `username.pragueconnect.eth.limo`
...
**Hero demo:** judge claims `judgename.pragueconnect.eth` → site goes live at `judgename.pragueconnect.eth.limo` → judge "tips" the booth via the stealth tip-jar → block explorer side-by-side shows the recipient address is unlinkable.
...
- **Delegation patterns where an agent acts on behalf of a named human principal** ← matches PragueConnect's "optional delegate" framing exactly

**Native fit:** reachable as a stretch goal. PLAN previously called agents *optional*; this bounty makes the agent layer worth ~$2k.

**Concrete shape:**
- Each user's optional PragueConnect agent gets `agent.kilian.pragueconnect.eth`
- Text records: `capabilities`, `model`, `endpoint`, `policy-hash`, `principal=kilian.pragueconnect.eth`
- Agent-to-agent: bob's agent resolves `kilian.pragueconnect.eth` → finds agent endpoint → negotiates a gig → both sign escrow
...
**Native fit:** essentially what PragueConnect is. Real money moves (escrow micro-payments), identity *is* the product (reputation lives on the human), peer-to-peer with no central rent-seeker. Stealth payments + ZK reputation + non-custodial escrow = direct mapping.

**Sharpen to win:**
- Stage line: *"TaskRabbit can see every gig and every dollar; PragueConnect literally cannot."*
...
**Hero demo:** Kč 200 task escrowed → ZK rep proof shown before acceptance → completed → released to a stealth address → block explorer can't link recipient to `kilian.pragueconnect.eth`.

### 4. Best UX Flow bounty — *load-bearing, prescriptive rubric*

**Confirmed rubric (their bullets, mapped to PragueConnect delivery):**

| Requirement | Where PragueConnect delivers it |
|---|---|
...
**Hero demo:** judge A messages judge B. Show platform DB — ciphertext. Then judge B presents a ZK proof of reputation to A without revealing history. Side-by-side: TaskRabbit's surveillance dashboard vs. PragueConnect's empty one.
...
| Agent layer adds scope vs. "humans first" framing | Agent is opt-in `agent.kilian.pragueconnect.eth` subname acting under explicit delegation with scoped permissions — leans into both ENS Bounty 1 ($2k) and EthUX agent rubric. |
...
| Load-bearing | **Network Economy track** | track | High (PragueConnect thesis matches sponsor framing exactly) |
...
| Reachable | **ENS Bounty 1 — AI Agents** | $1,250 / $750 | Medium-high (requires shipping the optional agent layer with `agent.x.pragueconnect.eth`) |
...
**Day 1** — identity loop. NameStone hosted offchain resolver mints `username.pragueconnect.eth` on first login. Privy embedded wallet wraps it with email/SMS auth. Contenthash → IPNS pointer → static personal site bundle published to IPFS, served at `username.pragueconnect.eth.limo`. Pimlico paymaster URL set in Privy dashboard sponsors all txs. Hero: judge claims a name, sees their page live in 30s.
...
**Day 5** — agent layer (ENS Bounty 1) + UX rubric polish. `agent.username.pragueconnect.eth` subname + ENSIP-25 text record + ERC-8004 Identity Registry on Sepolia + custom `delegation-policy` text record. ERC-7730 V2 descriptors for PragueConnect's contracts (anti-blind-signing). Delayed-broadcast Undo (~5-10s client-side buffer). i18next shell with `cs.json` + `en.json`.

**Day 6** — polish. The 90-second demo timing. Magnum Opus animations. Walk through ethux.design checklists as self-audit.

**Day 7** — demo. Hand a phone cold to a non-crypto judge. Stage the block-explorer side-by-side moment for the stealth payment ("TaskRabbit can see every dollar; PragueConnect literally cannot").
...
- [x] **Anti-blind-signing: ERC-7730 V2** descriptors (Ledger maintains the open registry; we ship our own JSON for PragueConnect contracts).
...
- [x] **Privacy stack to namecheck:** Web3Privacy Now / Privacy Builder Stack at build.web3privacy.info — cite XMTP-MLS, Semaphore, ERC-5564 as our chosen Builder Stack subset; place PragueConnect in the Marketplace category (currently white space — no W3PN-listed project covers stealth-payments-as-marketplace-rail).
- [x] **Visual direction:** Rudolfine Workshop (locked, design canvas delivered, see [`design/`](./design/)).
- [ ] **Day-1 booth conversations:** workemon.eth (TG: workemon) on ENS+AI agents (ENSIP-25 reference implementation questions); NameStone team for parent `pragueconnect.eth` provisioning.
- [x] **Scaffold complete (2026-05-08):** see [`README.md`](./README.md). Next.js 16 app at `web/` with **all 12 screens ported** as 13 routes, Privy/wagmi/Pimlico providers wired (demo-mode fallback), NameStone client, FluidKey + ScopeLift stealth wrappers. Foundry contracts at `contracts/` — `PragueConnectEscrow.sol` with Magnum Opus four-phase state machine, stealth payout to ERC-5564 announcer, reputation commitment hash, **7/7 tests passing**. ERC-7730 V2 descriptor at `contracts/erc7730/PragueConnectEscrow.json`. Build clean, dev server boots, all routes return 200.
- [x] **On-chain milestones (2026-05-08):**
  - **`pragueconnect.eth` registered on Sepolia ENS** — owner `0x2908…9D10` (open-agents deployer key), resolver `0x8FADE…B7dD` (Sepolia PublicResolver), 1-yr rent. Register tx: `0x38ee44a3…b93861fc`. Sepolia balance after: 0.0966 ETH.
  - **`PragueConnectEscrow` deployed on Base Sepolia at `0xcec992ABAfA04cD2F0c89BFAa93bdae3bF9da67F`** — wired to canonical ScopeLift announcer `0x55649E…45564`. Base Sepolia balance after: 0.0099 ETH.
  - `web/.env.local` populated with deployed addresses + chain config.
  - **NameStone authorized for `pragueconnect.eth` (Sepolia).** Resolver tx `0x8645fd08…ccea6e` set the parent's resolver to NameStone's Sepolia resolver `0xA87361…715125`. SIWE-signed enable-domain handshake completed against `https://namestone.com/api/public_v1_sepolia` (note: NameStone's testnet base URL is `_sepolia` suffix on `public_v1`, not a query param — undocumented in the public API page, found via SDK source `namestonehq/namestone-sdk/src/client.ts`).
  - **10 demo subnames issued** under `pragueconnect.eth` (kilian, lucia, bohuslav, milena, tomas, pavla, jirka, radek, ales, jan), each with `name`/`location` text records; kilian carries full bio + avatar URL + description + twitter handle for the hero demo.
  - **Vercel production live** at https://pragueconnect-azure.vercel.app with Privy + Pimlico + NameStone all wired.
- [ ] **Pre-hackathon (this week):** register `pragueconnect.eth` on L1, set NameStone resolver, deploy `PragueConnectEscrow` to Base Sepolia (`forge script script/Deploy.s.sol`), deploy ERC-8004 registry on Sepolia, create Privy app + Pimlico project + Base RPC keys → fill `.env.local`.
