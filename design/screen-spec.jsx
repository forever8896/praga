// Screen 11 — System spec sheet (one illuminated leaf)

function ScreenSpec() {
  return (
    <div className="parchment-surface" style={{ width: '100%', minHeight: '100%', padding: '64px 80px', position: 'relative', fontFamily: 'var(--body)', color: 'var(--ink)' }}>
      {/* masthead */}
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 32 }}>
        <div style={{ flex: 1 }}>
          ...
          <div className="t-italic" style={{ fontSize: 17, color: 'var(--ink-70)', marginTop: 8, maxWidth: 560 }}>Pigments, type, sigils, ornaments, seals — every part the craftsman keeps within reach.</div>
        </div>
      ...
      <SpecBlock number="I" title="Pigments — not light" sub="No gradients. No neon. Color comes from the workshop, not the screen.">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 18 }}>
          {[
            { name: 'Ink', hex: '#0F0E0C', role: 'Text · primary surface in dark', cssvar: '--ink' },
            { name: 'Parchment', hex: '#F4ECD8', role: 'Primary background', cssvar: '--parchment' },
            { name: 'Bone', hex: '#E6DCC4', role: 'Card lift on parchment', cssvar: '--bone' },
            { name: 'Vermilion', hex: '#B23A2F', role: 'Bohemian seal red · primary accent', cssvar: '--vermilion' },
            { name: 'Lapis', hex: '#1F3A6E', role: 'Royal blue · links', cssvar: '--lapis' },
            { name: 'Gilded', hex: '#C8A24A', role: 'Hairlines · hallmarks · fleur', cssvar: '--gilded' },
            { name: 'Verdigris', hex: '#3E6B5A', role: 'Bohemian crystal · success', cssvar: '--verdigris' },
          ].map(p => (
            <div key={p.name}>
      ...
      <SpecBlock number="II" title="Type — the chiseled, the hand, the metal" sub="Four registers. Each does typographic work or is removed.">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
          ...
          <SpecTypeRow label="Body · EB Garamond" role="All human-written copy · italic real, descenders long">
            ...
            <div className="t-italic" style={{ fontSize: 18, lineHeight: 1.5, marginTop: 8, color: 'var(--ink-70)' }}>Bring it past the church on Krásova; the door is the green one with the brass bell.</div>
          </SpecTypeRow>
      ...
      <SpecBlock number="IV" title="Fleur-de-lis — four variants, no wallpaper" sub="Maximum two per screen on mobile. Four on desktop. If it isn't earning its place, remove it.">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20 }}>
          {[
            ['Standard', 'gilded', 'parchment'],
            ['Inverted', 'parchment', 'ink'],
            ['Vermilion', 'vermilion', 'parchment'],
            ['Verified', 'gilded', 'wax'],
          ].map(([n, fg, bg], i) => (
            <div key={n} style={{ aspectRatio: '1.4 / 1', background: bg === 'ink' ? 'var(--ink)' : bg === 'wax' ? 'var(--vermilion)' : 'transparent', border: '0.5px solid var(--gilded)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, padding: 12 }}>
              <FleurDeLis size={48} stroke={fg === 'gilded' ? 'var(--gilded)' : fg === 'parchment' ? 'var(--parchment)' : 'var(--vermilion)'} />
              <div className="t-display" style={{ fontSize: 11, letterSpacing: '0.2em', color: bg === 'ink' || bg === 'wax' ? 'var(--parchment)' : 'var(--ink)' }}>{n}</div>
            </div>
          ))}
        </div>
      </SpecBlock>
      ...
      <SpecBlock number="VI" title="Cartouche &amp; marginalia" sub="Cartouches frame profile cards and reputation receipts. Marginalia ride only the desktop margins.">
        <div style={{ display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: 28 }}>
          <Cartouche style={{ background: 'var(--bone)', minHeight: 240 }} padding={36}>
            ...
            <div className="t-italic" style={{ fontSize: 14, marginTop: 14, lineHeight: 1.5, maxWidth: 420, color: 'var(--ink-70)' }}>The asymmetric scrollwork is engraved in 0.5pt gilded hairline. The frame should look chiseled into the page, not stickered onto it.</div>
          </Cartouche>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 14 }}>
        <div className="t-italic" style={{ fontSize: 13, color: 'var(--ink-70)' }}>Banned in copy: <span className="t-mono" style={{ fontSize: 12 }}>wallet · gas · nonce · tx · hash · mint · revert · network · chain · address</span>. Replace with: account, fee (sponsored), receipt, attestation, sealed, returned, ledger, name.</div>
        <FleurDeLis size={20} />
      </div>
    </div>
  );
...
function SpecBlock({ number, title, sub, children }) {
  return (
...
function SpecTypeRow({ label, role, children }) {
  return (
    <div>
...
Object.assign(window, { ScreenSpec });
