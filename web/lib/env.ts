...
export const env = {
  ...
  // Praga's own deployed addresses — empty until contracts/script/Deploy.s.sol is run.
  escrowAddress: process.env.NEXT_PUBLIC_PRAGA_ESCROW_ADDRESS ?? "",
  tipAddress: (process.env.NEXT_PUBLIC_PRAGA_TIP_ADDRESS ?? "") as `0x${string}` | "",
