...
3. **Compose → Feed.** `/compose` writes an `offers` JSON text record on the user's subname. `/feed` server-fetches every subname under `pragueconnect.eth`, flattens the offers, and ranks by `posted_at`. Filterable by sigil chips. No platform DB.
4. **Tip → Stealth payment.** `/tip/<ens>` reads the recipient's `stealth-meta-address`, derives a fresh stealth address client-side, and calls `PragueConnectTip.tip(stealth, ephem, viewTag, memo)` on Base Sepolia. The contract atomically transfers ETH and announces via the canonical ScopeLift ERC-5564 announcer (`0x5564…`). Block-explorer view ≠ profile view; the link is broken at the speed of one transaction.
...
│       ├── escrow.ts               PragueConnectEscrow ABI + taskId derivation + read helpers
...
└── contracts/                      Foundry
    ...
    │   └── PragueConnectTip.sol            One-shot stealth gift (single tx: transfer + announce)
    ├── test/                       7/7 passing
...
|---|---|---|
| `PragueConnectEscrow` | `0xcec992ABAfA04cD2F0c89BFAa93bdae3bF9da67F` | Magnum Opus four-phase |
| `PragueConnectTip` | `0x44b09471Ece57443FE5b9d350eF3B4f90244AB1b` | One-shot stealth gift |
...
| `pragueconnect.eth` | Sepolia ENS, NameStone resolver | All subnames |
...
1. **Open** `https://pragueconnect-azure.vercel.app` on a phone. Notice CS/EN toggle in the top right.
...
| **ENS Bounty 1 — AI Agents** (stretch) | `agent.<label>.pragueconnect.eth` delegation pattern + ENSIP-25 + ERC-8004 — UI mock present, full flow scoped for post-hackathon |

## Stack (locked 2026-05-08, all shipped)

- **ENS subnames:** NameStone hosted offchain resolver on `pragueconnect.eth`
